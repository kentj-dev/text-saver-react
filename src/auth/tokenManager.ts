...
export function parseTokenSet(
  ...
  if (!accessToken) {
    throw new ApiError(
      "The licensing server returned incomplete device credentials.",
  ...
  const refreshExpiresIn = Number(
    (source as TokenPayload & { refresh_token_expires_in?: number })
  ...
  return {
    ...
    accessTokenExpiresAt:
      timestamp(source.access_token_expires_at) ||
      Date.now() +
        (Number.isFinite(expiresIn) && expiresIn > 0
          ? expiresIn * 1000
          : 15 * 60 * 1000),
    refreshTokenExpiresAt:
      timestamp(source.refresh_token_expires_at) ||
...
export class TokenManager {
  ...
  async getAccessToken(): Promise<string> {
    ...
    if (!session?.tokens)
      throw new ApiError("Activate Plus to continue.", "AUTH_REQUIRED", 401);
  ...
  async refreshAfterUnauthorized(
    ...
    if (current.tokens.accessToken !== rejectedAccessToken)
  ...
  private async coordinateRefresh(): Promise<TokenSet> {
    ...
    if (!expected?.tokens?.refreshToken)
      throw new ApiError("Activate Plus to continue.", "AUTH_REQUIRED", 401);
    const execute = async () => {
      const current = await this.storage.getSession();
      if (!current?.tokens)
  ...
  private async performRefresh(session: AuthSession): Promise<TokenSet> {
    ...
    try {
      response = await fetch(
        `${BILLING_CONFIG.apiBaseUrl.replace(/\/$/, "")}/auth/refresh`,
        {
          method: "POST",
          headers: {
    ...
    try {
      ...
      const error =
        unknownError instanceof ApiError
          ? unknownError
          : new ApiError(
              "The session could not be renewed safely.",
              "ROTATION_REQUIRED",
              401,
            );
      await this.recordRefreshFailure(session, error);
    ...
    if (!current.tokens)
      throw new ApiError("Activate Plus to continue.", "AUTH_REQUIRED", 401);
    if (current.tokens?.refreshToken !== session.tokens.refreshToken)
      return current.tokens;
    ...
    try {
      if (payload.entitlements || payload.license)
        entitlements = entitlementService.fromPayload(payload);
    } catch (unknownError) {
      const error =
        unknownError instanceof ApiError
          ? unknownError
          : new ApiError(
              "The entitlement response was invalid.",
              "BACKEND_CONTRACT_ERROR",
              502,
            );
      await this.storage.setSession({
        ...rotatedSession,
        status: entitlementService.isOfflineGraceAvailable(rotatedSession)
          ? "offline_grace"
          : "offline_locked",
    ...
    await this.storage.setSession({
      ...rotatedSession,
      status:
        entitlements?.licenseStatus?.toLowerCase() === "expired"
          ? "expired"
          : "active",
      entitlements,
      lastValidatedAt: now,
      offlineGraceUntil: entitlements
        ? entitlementService.offlineGraceUntil(entitlements, now)
        : rotatedSession.offlineGraceUntil,
      errorCode: undefined,
    });
    return tokens;
  }
  ...
  private async recordRefreshFailure(
    ...
    if (
      error.isTemporary &&
      entitlementService.isOfflineGraceAvailable(session)
    ) {
      await this.storage.setSession({
        ...session,
