...
function serializedError(error: unknown) {
  ...
  return {
    message:
      error instanceof Error ? error.message : "Licensing request failed.",
    code: "REQUEST_FAILED",
    status: 0,
  };
}

export async function runLicensingCommand(
  action: string,
  ...
  switch (action) {
      ...
      return {
        ...
        devices: Array.isArray(result.devices)
          ? result.devices.map((device) => ({
              id: String(device.id),
              deviceName:
                device.device_name == null ? null : String(device.device_name),
              platform:
                device.platform == null ? null : String(device.platform),
              appVersion:
                device.app_version == null ? null : String(device.app_version),
    ...
    case "deactivateDevice":
      await apiClient.request(
        `/devices/${encodeURIComponent(String(payload.id || ""))}`,
        { method: "DELETE" },
      );
      return null;
    case "revokeDevice":
      await apiClient.request(
        `/devices/${encodeURIComponent(String(payload.id || ""))}/revoke`,
        { method: "POST" },
      );
      return null;
    case "request": {
...
export async function handleLicensingMessage(message: unknown) {
  ...
  try {
    return {
      ok: true,
      result: await runLicensingCommand(
