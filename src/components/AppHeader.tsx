...
import { Cloud, Ellipsis, FileDown, FileUp, HelpCircle, KeyRound, Lock, Moon, Pencil, Search, Sun, Trash2, Unlock } from 'lucide-react';
    ...
    <header className="flex min-h-12 items-center justify-between border-b border-border pb-2.5">
      <div className="brand flex items-center gap-2.5" data-tour="welcome">
        ...
        <div>
          ...
          <div className="text-[10px] uppercase tracking-[.18em] text-muted-foreground">{props.planName} plan</div>
        </div>
      </div>
      <div className="flex items-center gap-0.5">
        <span className={cn('mr-1 text-[11px] text-muted-foreground', props.saveStatus === 'error' && 'text-destructive')}>
          {props.saveStatus === 'saving' ? 'Saving…' : props.saveStatus === 'error' ? 'Save failed' : ''}
        </span>
        <Button variant="ghost" size="icon" title={`Switch to ${props.theme === 'dark' ? 'light' : 'dark'} mode`} onClick={props.onToggleTheme}>
          {props.theme === 'dark' ? <Sun /> : <Moon />}
        </Button>
        <Button variant="ghost" size="icon" title="Find in tab" data-tour="find" onClick={props.onOpenFind}><Search /></Button>
        {!isInbox(props.activeTab) && (
          <>
            <Button variant="ghost" size="icon" data-tour="security" title={isEncryptedTab(props.activeTab) ? (props.locked ? 'Unlock tab' : 'Password options') : 'Set password'} onClick={() => props.onSecurity(props.activeTab.id)}>
              {isEncryptedTab(props.activeTab) && props.locked ? <Unlock /> : <Lock />}
            </Button>
            <Button variant="ghost" size="icon" title={`Rename ${props.activeTab.name}`} onClick={() => props.onRename(props.activeTab.id)}><Pencil /></Button>
            <Button variant="ghost" size="icon" title={`Delete ${props.activeTab.name}`} disabled={props.normalTabCount === 1} onClick={() => props.onDelete(props.activeTab.id)}><Trash2 /></Button>
          </>
        )}
        <DropdownMenu>
          <DropdownMenuTrigger asChild><Button variant="ghost" size="icon" aria-label="More options" data-tour="more"><Ellipsis /></Button></DropdownMenuTrigger>
          <DropdownMenuContent align="end">
            <DropdownMenuItem onSelect={props.onOpenPlans}><Cloud />Plan &amp; cloud</DropdownMenuItem>
            <DropdownMenuItem onSelect={props.onOpenGuide}><HelpCircle />Guide</DropdownMenuItem>
            <DropdownMenuSeparator />
            <DropdownMenuItem onSelect={props.onExportBackup}><FileUp />Export backup</DropdownMenuItem>
