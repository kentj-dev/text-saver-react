  ...
  return (
    <div className="flex items-center justify-between gap-3 rounded-md bg-muted/50 px-2.5 py-2">
      <div className="flex min-w-0 items-center gap-2">
        {item.protected ? (
          <FileLock2 className="size-4 shrink-0 text-amber-500" />
        ) : (
...
function PromptDialogView({ config, onResolve }: Props) {
  ...
  async function submit() {
    ...
    try {
      ...
      onResolve(
        config.inputTwo ? [first, second] : config.input ? first : true,
      );
    } finally {
      setBusy(false);
    }
  ...
  return (
    <Dialog
      open={Boolean(config)}
      onOpenChange={(open) => !open && onResolve(null)}
    >
      <DialogContent
        hideClose
        onKeyDown={(event) => {
        ...
        {config?.input && (
          <label className="grid gap-1.5 text-[13px] text-muted-foreground">
            ...
            <Input
              ...
              autoComplete={config.input.autocomplete || "off"}
              maxLength={120}
            />
          </label>
        )}
        {config?.inputTwo && (
          <label className="grid gap-1.5 text-[13px] text-muted-foreground">
            ...
            <Input
              ...
              type={config.inputTwo.type || "text"}
              autoComplete={config.inputTwo.autocomplete || "off"}
              maxLength={120}
            />
          </label>
        )}
...
export function PromptDialog() {
  ...
  return (
    <PromptDialogView
      config={ui.promptConfig}
      onResolve={actions.resolvePrompt}
    />
  );
}
