...
export function isEncryptedTab(
  tab: SaverTab | undefined | null,
): tab is EncryptedTab {
...
export function licenseStatusText(license: License | null) {
  ...
  if (license.status === "refreshing")
    return "Securely renewing this device session…";
  if (license.status === "device_revoked")
    return "This device was deactivated. Your local notes remain available.";
  if (license.status === "revoked")
    return "This license was revoked. Your local notes remain available.";
  ...
  if (
    license.status === "subscription_expired" ||
  ...
  if (license.status !== "active")
    return "No license is active on this installation.";
  const checkedAt = license.lastValidatedAt
