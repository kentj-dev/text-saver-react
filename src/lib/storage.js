...
// A corruption/safety ceiling, deliberately separate from entitlement limits.
// Paid-plan enforcement happens in the UI and never makes existing data invalid.
export const MAX_USER_TABS = 200;
...
export function createPlainTab(name, text = "", kind = "normal") {
  return {
    id: kind === "inbox" ? INBOX_ID : createId(),
    name,
    kind,
    protected: false,
    text,
...
export function isValidTab(tab) {
  if (!tab || typeof tab.id !== "string" || typeof tab.name !== "string")
    return false;
  ...
  return (
    ...
    (tab.encryptionContextId === undefined ||
      typeof tab.encryptionContextId === "string")
  );
}

export function isValidState(value) {
  if (!value || value.version !== SCHEMA_VERSION || !Array.isArray(value.tabs))
    return false;
  if (!value.tabs.length || !value.tabs.every(isValidTab)) return false;
  if (value.tabs.filter((tab) => tab.kind === "normal").length > MAX_USER_TABS)
    return false;
...
export async function getOrMigrateState(storage = chrome.storage.local) {
  ...
  if (existing !== undefined && !isVersionOneState(existing)) {
    throw new Error(
      "Saved Text Saver data has an unsupported or malformed schema.",
    );
  }
  ...
  if (isVersionOneState(existing)) {
    ...
    const legacyText =
      typeof stored[LEGACY_KEY] === "string" ? stored[LEGACY_KEY] : "";
...
export async function saveState(state, storage = chrome.storage.local) {
  if (!isValidState(state))
    throw new Error("Refusing to save invalid Text Saver data.");
  await storage.set({ [STATE_KEY]: state });
}
...
function additionalData(encryptionContextId) {
  return encoder.encode(`text-saver:v${SCHEMA_VERSION}:${encryptionContextId}`);
}
  ...
  const ciphertext = await crypto.subtle.encrypt(
    {
      name: "AES-GCM",
      iv,
      additionalData: additionalData(encryptionContextId),
      tagLength: 128,
    },
    key,
    encoder.encode(text),
  ...
  const plaintext = await crypto.subtle.decrypt(
    {
      ...
      additionalData: additionalData(tab.encryptionContextId || tab.id),
      tagLength: 128,
    },
    key,
    base64ToBytes(tab.ciphertext),
  );
  return decoder.decode(plaintext);
}

export async function cacheUnlockKey(
  tabId,
  keyBytes,
