...
export function licenseView(session) {
  ...
  return {
    ...
    planId: session.entitlements?.features?.includes("cloud_sync")
      ? "plus"
      : "free",
...
export function effectivePlanId(license, now = Date.now()) {
  ...
  if (license.status === "active" || license.status === "refreshing")
    return "plus";
  if (
    license.status === "offline_grace" &&
    Number(license.offlineValidUntil) > now
  )
    return "plus";
  return "free";
}
...
export async function ensureActiveLicense() {
  ...
  if (!license)
    throw new ApiError(
      "Activate Plus to use cloud sync.",
      "AUTH_REQUIRED",
      401,
    );
  if (
    !license.lastValidatedAt ||
    Date.now() - license.lastValidatedAt >= VALIDATION_INTERVAL_MS
  ) {
    license = await validateLicense({ force: true });
  }
  if (!license || effectivePlanId(license) !== "plus") {
    throw new ApiError(
      "Cloud sync requires an active Plus license.",
...
export async function licenseApiRequest(path, _license, body) {
  ...
  return invokeLicensing(
    "request",
    body === undefined ? { path } : { path, body },
  );
}
...
function restoreError(error) {
  return new ApiError(
    error?.message || "Licensing request failed.",
