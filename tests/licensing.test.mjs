...
test("ten simultaneous unauthorized requests cause one rotating refresh", async () => {
  ...
  const allOldRequestsStarted = new Promise((resolve) => {
  ...
  assert.equal(
    area.values[AUTH_SESSION_KEY].tokens.refreshToken,
    "refresh-new",
  );
...
test("a refresh response must rotate the refresh token", async () => {
  const area = memoryArea({
    [AUTH_SESSION_KEY]: activeSession({
      tokens: {
        accessToken: "expired",
...
test("rotated tokens are saved even when refresh entitlement metadata is invalid", async () => {
  ...
  await assert.rejects(
    () => manager.refresh(),
    (error) => error.code === "PRODUCT_MISMATCH",
  );
...
test("device revocation clears bearer credentials and records a customer-facing state", async () => {
  ...
  const manager = new TokenManager(new SecureStorage(area));
  globalThis.fetch = async () =>
    Response.json({ error: "DEVICE_REVOKED" }, { status: 401 });
...
test("a definite expired response locks cached entitlements without discarding refresh credentials", async () => {
  ...
  globalThis.fetch = async () =>
    Response.json({ error: "license_expired" }, { status: 403 });

  await assert.rejects(
    () =>
      client.request("/sync/push", {
        method: "POST",
        requiredEntitlement: "cloud_sync",
        body: { revision: 0, data: "ciphertext" },
      }),
...
test("legacy raw license storage is purged rather than migrated", async () => {
  const area = memoryArea({
    [LEGACY_LICENSE_KEY]: { licenseKey: "raw-customer-key" },
  });
  const storage = new SecureStorage(area);
