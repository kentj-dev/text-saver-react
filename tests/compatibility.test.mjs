...
import {
  AUTH_SESSION_KEY,
  LEGACY_LICENSE_KEY,
} from "../src/storage/secureStorage.ts";

const inbox = {
  id: "context-menu-inbox",
  name: "Context Menu",
  kind: "inbox",
  protected: false,
  text: "",
...
test("React build keeps the published plan limits and entitlement rules", () => {
  ...
  assert.equal(
    effectivePlanId({
      ...
      offlineValidUntil: Date.now() + 1000,
    }),
    "plus",
  );
  assert.equal(
    effectivePlanId({
      plan: "Any backend plan name",
      features: ["cloud_sync"],
      status: "active",
    }),
    "plus",
  ...
  assert.equal(
    effectivePlanId({ plan: "Plus", features: [], status: "active" }),
    "free",
  );
});
...
test("a new device receives synced tabs without losing its local tabs", () => {
  ...
  assert.equal(
    copied.tabs.find((item) => item.id === "shared").text,
    "from device one",
  );
});
...
test("selective sync merges edits while leaving unselected tabs local", () => {
  ...
  const localState = state([
    tab("local-only", "untouched"),
    tab("shared", "local edit"),
  ]);
  ...
  const applied = applySyncDocument(
    localState,
    merged.document,
    merged.conflicts,
  );
  assert.equal(
    applied.tabs.find((item) => item.id === "shared").text,
    "local edit",
  );
  assert.equal(
    applied.tabs.find((item) => item.id === "local-only").text,
    "untouched",
  );
});
...
test("activation stores device tokens but permanently discards the raw license key", async () => {
  ...
  assert.equal(
    request.url,
    "https://apps.hamiken.com/api/v1/licenses/activate",
  );
  ...
  assert.equal(
    values[AUTH_SESSION_KEY].tokens.accessToken,
    "short-lived-access-token",
  );
  assert.equal(
    values[AUTH_SESSION_KEY].tokens.refreshToken,
  ...
  globalThis.chrome = {
    runtime: {
      sendMessage: async (value) => {
        ...
        return {
          ok: true,
          result: { status: "active", features: ["cloud_sync"] },
        };
      },
    },
  };
  globalThis.fetch = async () => {
    throw new Error("popup must not fetch licensing routes");
  };
...
test("React build accepts the existing version-two storage schema", () => {
  assert.equal(
    isValidState(state([tab("existing-tab", "Existing user data")])),
    true,
  );
});
...
test("React build preserves three-way sync conflict behavior", () => {
  ...
  assert.deepEqual(
    new Set(normalTabs.map((item) => item.text)),
